...
function generateLicenseKey() {
  ...
  return `${prefix}-${random.slice(0, 4)}-${random.slice(4, 8)}-${random.slice(
    8,
    12
  )}-${random.slice(12, 16)}`;
}
...
app.post("/api/validate-license", async (req, res) => {
  try {
    ...
    // Check device limit
    const deviceIndex = license.devices.findIndex(
...
app.post("/api/admin/create-license", async (req, res) => {
  try {
    const { adminApiKey, email, maxDevices, durationDays, appsheetsUrl } =
      req.body;
    ...
    const license = await License.create({
      ...
      appsheetsUrl: appsheetsUrl || null,
      devices: [],
    });
...
//   const runTask = await License.updateMany(
//     { appSheetsUrl: { $exists: false } },
//     { $set: { appSheetsUrl: null } }
//   );
//   if (runTask.modifiedCount > 0) {
//     console.log(`${runTask.modifiedCount} licenses updated`);
//   }
// };
